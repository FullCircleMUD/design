...
| [library-standards.md](library-standards.md) | Conventions for the libraries under `libraries/` — the `evennia-` / `fcm-` families, naming, src layout, `pyproject.toml`, test framework, the reduced sub-repo documentation set, and the bootstrap checklist. |
| [test-first-process.md](test-first-process.md) | The standard development order across every FCM repo — test plan defines the behaviour, then the tests (with `NotImplementedError` placeholders where needed), then the code that makes them pass. |
